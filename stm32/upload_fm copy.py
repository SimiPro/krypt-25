...
class STM32RDPDemo:
    def __init__(self, firmware_path=None):
        ...
        self.eeprom_bank1_start = "0x08080000"
        self.eeprom_bank1_end = "0x08080BFF"   # 3KB
        self.eeprom_bank2_start = "0x08080C00" 
        self.eeprom_bank2_end = "0x080817FF"    # 3KB
        
        # Total EEPROM size (both banks)
        self.eeprom_size = "0x1800"  # 6KB total

        # memo mode = 0, Boot0 = 0, UFB = 0
        self.bank_1_start = "0x08000000"  # Flash start address
        self.bank_1_end = "0x08017FFF"
        self.bank_2_start = "0x08018000"
        self.bank_2_end = "0x0802FFFF"

        # memo mode = 0, Boot0 = 0, UFB = 1
        # self.bank_1_start = "0x08018000"  # Flash start address
        # self.bank_1_end = "x0802FFFF"
        # self.bank_2_start = "0x08018000"
        # self.bank_2_end = "0x0802FFFF"


        self.option_bytes_address = "0x1FF80000"

        self.read_size = "0x18000"  #  96kbytes
    ...
    def get_device_info(self):
        ...
        cmd = ['st-info', '--probe']
        return self.run_command(cmd, "Getting device information")

    def read_option_bytes(self):
        """Read current option bytes to check RDP level"""
        cmd = ['st-flash', '--format', 'binary', 'read', 'option_bytes.bin', self.option_bytes_address, '0x10']
        result = self.run_command(cmd, "Reading option bytes")
        ...
        if result and os.path.exists('option_bytes.bin'):
            try:
                ...
                print(f"Current RDP Level: 0x{rdp_byte:02X}")
                if rdp_byte == 0xAA:
    ...
    def upload_firmware(self):
        ...
        if not os.path.exists(self.firmware_path):
            print(f"ERROR: Firmware file {self.firmware_path} not found!")
            return False
            
        cmd = ['st-flash', 'write', self.firmware_path, '0x8000000']
        result = self.run_command(cmd, f"Uploading firmware: {self.firmware_path}")
        return result is not None

    def create_test_firmware(self):
        """Create a simple test firmware binary"""
        self.firmware_path = "test_firmware.bin"
        ...
        print(f"Created test firmware: {self.firmware_path}")

    def read_flash(self, description=""):
        """Read flash memory"""
        output_file = f"flash_read_{int(time.time())}.bin"
        cmd = ['st-flash', '--format', 'binary', 'read', output_file, self.eeprom_bank1_start, self.eeprom_size]
        result = self.run_command(cmd, f"Reading flash memory {description}")
    ...
    def set_rdp_level_1(self):
        ...
        current_rdp = self.read_option_bytes()
        ...
        try:
            ...
            cmd = ['st-flash', '--format', 'binary', 'write', 'new_option_bytes.bin', self.option_bytes_address]
    ...
    def reset_device(self):
        """Reset the device"""
        cmd = ['st-flash', 'reset']
        return self.run_command(cmd, "Resetting device")

    def run_demo(self):
        """Run the complete RDP demonstration"""
        ...
        print("\n" + "="*50)
        print("STEP 1: Check current RDP level")
        print("="*50)
        current_rdp = self.read_option_bytes()

        input("Press Enter to continue...")

        if not self.set_rdp_level_0():
        ...
        print("="*50)
        data_before, all_zeros_before = self.read_flash("(before RDP)")
        ...
        print("="*50)
        new_rdp = self.read_option_bytes()
        
        # Try to read flash after RDP
        print("\n" + "="*50)
        print("STEP 6: Read flash AFTER RDP protection")
        print("="*50)
        data_after, all_zeros_after = self.read_flash("(after RDP)")
...
def main():
    if len(sys.argv) > 1:
        ...
        firmware_path = None
        print("No firmware file specified. Will create test firmware.")
    
    demo = STM32RDPDemo(firmware_path)
